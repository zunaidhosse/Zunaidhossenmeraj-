...
const ReceivableDetail: React.FC = () => {
    ...
    const receivable = context?.receivables.find(r => r.id === id);

    if (!context || !receivable) {
        return (
            <div className="p-4 text-center">
                <p>Receivable not found.</p>
                <button onClick={() => navigate('/receivables')} className="mt-4 text-primary">Go Back</button>
            </div>
        );
    }

    const { deleteReceivable, addPayment, settings } = context;
    
    const paidAmount = (receivable.payments || []).reduce((sum, p) => sum + p.amount, 0);
    const remainingAmount = receivable.amount - paidAmount;
    const progress = receivable.amount > 0 ? (paidAmount / receivable.amount) * 100 : 100;
        ...
        <div className="p-4 space-y-6 pb-20">
            <header className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-xl font-bold truncate">{receivable.personName}</h1>
                </div>
                 <div className="flex items-center gap-2">
                    <Link to={`/receivables/edit/${receivable.id}`} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700">
                        <Edit size={20} />
                    </Link>
            ...
            <Card>
                <div className="space-y-3">
                    <div className="text-center">
                        ...
                        <p className="text-4xl font-bold text-primary">{remainingAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })} {settings.currency}</p>
                    </div>
                    ...
                    <div className="flex justify-between text-sm">
                        ...
                        <span className="text-gray-500 dark:text-gray-400">Total: {receivable.amount.toLocaleString()}</span>
                    </div>
                </div>
            </Card>
            ...
            {receivable.status === 'pending' && (
                 <Card>
                    <h3 className="font-semibold mb-4 text-lg">Add Payment</h3>
            ...
            <Card>
                ...
                {(receivable.payments || []).length > 0 ? (
                    <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                        {[...(receivable.payments || [])].reverse().map(p => (
